...
#+BEGIN_SRC python :results output :exports both :eval no-export
  ...
  eps = 0.0001 
  n = konvergenca(eps)
  print("eps = %f, n = %d (v %f sekundah)" % (eps, n, time.time() - zacetek))
...
Tudi funkcije, kot jih poznamo v Pythonu, so neke vrste predpisi, ki za vhodne
argumentom vrne določeno vrednost. Seveda pa funkcije v Pythonu lahko delajo
...
program javi napako tipa ~AssertionError~.
...
Preprosto prenesemo vse člene na eno stran enačbe.
...
*** Sekantna metoda
S [[https://sl.wikipedia.org/wiki/Sekantna_metoda][sekantno metodo]] konstruiramo zaporedje približkov, ki se približujejo ničli
funkcije, tako da začnemo z dvema približkoma za ničlo. Nato izračunamo sekanto
skozi dve točki na grafu funkcije, ki ustrezata izbranima približkoma. Nov
približek dobimo kot presečišče sekante in x-osi. Postopek ponovimo z novim
približkom in enim od začetnih. Postopek nato ponavljamo, dokler vrednost
funkcije $f(x_n)$ v zadnjem približku $x_n$ ni manjši od vnaprej predpisanega
števila $\varepsilon$.
...
print("%f: Vrednost 2^0.5, ki jo vrne Python" % 2**0.5)
#+END_SRC

#+RESULTS:
: (1.4142135620573204, 4)
: 1.414214: Vrednost 2^0.5, ki jo vrne Python
