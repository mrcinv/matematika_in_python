...
Vsebino lahko preskusite tako, da si namestite okolje za programski jezik
[[https://www.python.org/][Python]]. Naloge so tudi na portalu [[https://www.projekt-tomo.si/course/15/][projekt Tomo]], kjer lahko izdelane rešitve tudi
zelo enostavno preskusite. Z reševanjem teh nalog boste pridobili tako
...
$a_n=\frac{n-1}{n+1}$ za $n\in 1,2,\ldots$. Iz seznama izpiši člene $a_1$, $a_5$
in $a_{14}$.
**** Namig
     :PROPERTIES:
     :HTML_CONTAINER_CLASS: resitev
...
**** Rešitev 
    ...
    #+BEGIN_SRC python :results output :exports both :eval no-export
      ...
      for n in range(1,16):
          a.append((n-1)/(n+1))
      # izpišemo elemente seznama. Pazimo na to, da se indeksi začnejo z 0
      print("a_1 = %f, a_5 = %f, a_14 = %f" % (a[0], a[4], a[-2]))
    #+END_SRC
...
Opazimo, da se členi zaporedja po 5. členu začnejo ponavljati. Zaporedje je verjetno
*konvergentno*. Zadnjih nekaj členov je zelo dobrih približkov za *limito*.
**** Rešitev z generatorjem (napredno)
     :PROPERTIES:
     :HTML_CONTAINER_CLASS: resitev
     :END:
...
#+BEGIN_SRC python :results output :exports both :eval no-export
  def clen(n):
      """Rekurzivna funkcija, ki vrne n-ti člen zaporedja podanega z rekurzivno fomulo
      a(n+1) = (2*a(n)^3 + 2)/(3*a(n)^2)
      """
      if n<=0:
          return 2
      else:
...
časovni odvisnosti, je pri večkratnih klicih funkcije ~clen~ še vedno
precej izračunov, ki se vedno znova ponovijo. Ob vsakem zaporednem klicu funkcije ponovno
...
**** Rešitev z generatorjem
     ...
     :HTML_CONTAINER_CLASS: resitev
     :END:
Za ilustracijo nalogo rešimo še z *generatorjem*. Napisali bomo neskončen
generator, zato moramo v zanki posebej poskrbeti, da se le ta ustavi.
...
*** Vsota vrste
Za vrsto $$4\sum_{n=0}^\infty\left(-1\right)^n \frac{1}{1+2n}$$ je znano, da je
njena vsota enaka številu $\pi$. Preveri, da delne vsote te vrste res
konvergirajo k $\pi$. Napiši program (funkcijo) ~konvergenca~, ki za poljuben
$\varepsilon > 0$ poišče $n$, od katerega so vse delne vsote za manj kot
$\varepsilon$ oddaljene od $\pi$. Svoj program preskusi za
$\varepsilon=10^{-4}$. Kako majhen je lahko $\varepsilon$, da se program izvede v manj kot
nekaj sekundah (npr. 10s). 
...
#+BEGIN_SRC python :noweb strip-export :exports both :exports both :eval no-export
<<nicle>>
x = [-1, 0, 1, 2, 3]
y = [1, -1, 1, 2, 3]
print(nicle(x,y))
...
#+BEGIN_SRC python :noweb strip-export :exports both :eval no-export
  <<nicle>>
  import unittest
  import random
